...
class TranslatorEngine():
        ...
        with open(html_file, encoding='utf-8') as f:
            ...
            for ele in epub_eles:
                if isinstance(ele, element.NavigableString) and str(ele).strip() not in ['', 'html']:
                    if self.is_code_element(ele.parent):
                        code_list.append(str(ele))
                    else:
            ...
            for ele in epub_eles:
                if isinstance(ele, element.NavigableString) and str(ele).strip() not in ['', 'html']:
                    if self.is_code_element(ele.parent):
                        ele.replace_with(element.NavigableString(code_list.pop(0)))
                    else:
                        nextpos += 1
    ...
    def is_code_element(self, element):
        if element and element.name in ['pre', 'code']:
            return True
        elif element and element.parent:
            return self.is_code_element(element.parent)
        else:
            return False
